...
class Coupling(nn.Module):
    ...
    def inverse(self, u):
        mu = u * self.mask  # note that mu = mx, they are input that are not masked
        s = self.scale(mu)
        t = self.shift(mu)
...
# These functions take 6 inputs. q1(w1), q1(w2), q2(w1), q2(w2), logb=estimated log Bayes factor, pi=n1/(n1+n2)
...
class Discriminator(nn.Module):
    def __init__(self, logb, fdiv="js"):
    ...
    def forward(self, transformed_sample_at_transformed_density, transformed_sample_at_target_density,
        ...
        if self.fdiv == "js":
            ...
            print("sorry boss not implemented, try js or hellinger")
...
class TargetProposalDataset(torch.utils.data.Dataset):
    ...
    don't actually need the weight
    given index i, it
    """
...
class fGANBridge:
    def __init__(self, p1_sample, p1_log_density, p2_sample, p2_log_density,
        ...
        if self.fdiv == "js":
            ...
            print("sorry boss not implemented yet, try js or hellinger")
    ...
    def fit(self, n_block, hidden_size, hidden_layers, epoch=10, lr_generator=8e-4, lr_discriminator=5.,
        ...
        :param beta_for_kl1: additional log lkd term to stabilize f-GAN training
        :param beta_for_kl2: additional log lkd term to stabilize f-GAN training
        :param batch_norm: do we need batch norm? probably not lol
        ...
        :return: hopefully a good estimate of Bayes factor
        ...
        for EPOCH in range(epoch):
            ...
            for i, (data_batch, label_batch, weight_batch) in enumerate(self.training_loader):
                ...
                if i % 50 == 0:
                    with torch.no_grad():
                        print("THIS MATTERS: ", "Monitoring backward KL:",
                ...
                if i % k == 0:  # discriminator updates using batch data
                    ...
                    if i % 50 == 0:
                        print("Curr B: ", self.myDiscriminator.logb.detach().cpu())
            ...
            print("EPOCH: ", EPOCH, "TEST LOSS: ", test_obj[EPOCH], "Bridge B: ", Bseq[EPOCH])
        ...
        while curr_t < max_iter and rel_diff > 0.0001:
            ...
            old_r = new_r
            curr_t += 1
        # if optimal Bridge does not converge...
        # tired of implementing specific Bridge estimate, just search for the max lol
        ...
        if curr_t == max_iter:
            ...
            a1max = torch.max(a1)
            a2max = torch.max(a2)
            part1 = a1max + torch.log(torch.sum(torch.exp(a1-a1max)))
            part2 = a2max + torch.log(torch.mean(torch.exp(a2-a2max)))
            self.geometric_bridge = (part2 - part1 + torch.log(s1/(1-s1))).cpu().item()
            self.fitted_r = candidates[np.argmax(objective_value)].cpu().item()
            return self.fitted_r
        else:
            self.fitted_r = old_r.cpu().item()
            return self.fitted_r

    def divergence_estimation(self, p1_sample=None, p2_sample=None):
        """
        ...
        if p1_sample is None and p2_sample is None:
            ...
            if self.dim_diff != 0:
                augmented_p1_sample = torch.cat((p1_sample,
                                                 torch.randn((p1_sample.shape[0],
                                                              self.dim_diff), device=self.device)))
    ...
    def generate_plots(self, truth=None, lim=20):
        ...
        if truth is not None:
            ...
            print("RMSE is {}".format((self.fitted_r-truth)**2/truth**2))
    ...
    def new_bridge(self, q1_sample, q2_sample):
        ...
        rel_diff = 1.
        curr_t = 0
        while curr_t < 1500 and rel_diff > 0.0001:
            print(old_r)
